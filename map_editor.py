...
class MapEditor:
    ...
    def render(self) -> None:
        ...
        offset: Vector = Vector((-off_x * config.MAP_2D_SCALE) / config.MAP_2D_SCALE,
                                (-off_y * config.MAP_2D_SCALE) / config.MAP_2D_SCALE)
        ...
        # render spawn point
        engine.gfx.render_circle(self.map.spawn_point.x * config.MAP_2D_SCALE,
                                 self.map.spawn_point.y * config.MAP_2D_SCALE, 0.2 * config.MAP_2D_SCALE, (0, 0, 200))

        # render player if enabled
        if player is not None:
            engine.gfx.render_circle(half_width, half_height, 0.2 * config.MAP_2D_SCALE, (128, 128, 128))
            engine.gfx.render_line(half_width, half_height,
                                   half_width + player.frustum.far_left.x * config.MAP_2D_SCALE,
                                   half_height + player.frustum.far_left.y * config.MAP_2D_SCALE, (255, 255, 102))
            engine.gfx.render_line(half_width, half_height,
                                   half_width + player.frustum.far_right.x * config.MAP_2D_SCALE,
                                   half_height + player.frustum.far_right.y * config.MAP_2D_SCALE, (255, 255, 102))
